    ...
    internal class EEUI : UIState
    {
        ...
        public override void OnInitialize()
        {
            for (int i = 0; i < 9; i++)
            {
                ...
                if (i < 7)
                {
                    panels[i].Width.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Width, 0);
                    panels[i].Height.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Height, 0);
        ...
        private void ChooseFirstRune(UIMouseEvent evt, UIElement listeningElement)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                if (isPulsating[i])
                {
                    ...
                    panels[7]._backgroundTexture = ModContent.Request<Texture2D>(StringOfTextures[i]);
                    panels[7].Width.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Width, 0);
                    panels[7].Height.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Height, 0);
                    panels[7].Left.Set(Main.screenWidth * .5f - ModContent.Request<Texture2D>(StringOfTextures[i]).Width / 2 - 100, 0);
                    panels[7].Top.Set(500 - ModContent.Request<Texture2D>(StringOfTextures[i]).Height / 2, 0);
                    break;
                }
                else
                {
        ...
        private void ChooseSecondRune(UIMouseEvent evt, UIElement listeningElement)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                {
                    Main.LocalPlayer.GetModPlayer<EEPlayer>().inPossesion[i] = 1;
                    panels[8]._backgroundTexture = ModContent.Request<Texture2D>(StringOfTextures[i]);
                    panels[8].Width.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Width, 0);
                    panels[8].Height.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Height, 0);
                    panels[8].Left.Set(Main.screenWidth * .5f - ModContent.Request<Texture2D>(StringOfTextures[i]).Width / 2 + 100, 0);
                    panels[8].Top.Set(500 - +ModContent.Request<Texture2D>(StringOfTextures[i]).Height / 2, 0);
                    break;
                }
                else
                {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                panels[i].Left.Set(Main.screenWidth / 2 - ModContent.Request<Texture2D>(StringOfTextures[i]).Width / 2 + (float)Math.Sin(pivot / 10 + Math.PI * (i * 2) / 7) * 200, 0);
                panels[i].Top.Set(Main.screenHeight / 2 - ModContent.Request<Texture2D>(StringOfTextures[i]).Height / 2 + (float)Math.Cos(pivot / 10 + Math.PI * (i * 2) / 7) * 200, 0);
            }
            ...
            for (int i = 0; i < 7; i++)
            {
                if (isPulsating[i])
                {
                    panels[i].Width.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Width + (float)Math.Sin(pulsatingControl) * 2, 0);
                    panels[i].Height.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Height + (float)Math.Sin(pulsatingControl) * 2, 0);
                }
                else
                {
                    panels[i].Width.Set(ModContent.Request<Texture2D>(StringOfTextures[i]).Width, 0);
