    ...
    public class AquamarineLamp1Glow : EEProjectile
    {
                ...
                if (active)
                {
                    ...
                    if ((spire.ai[0] <= 40 && spire.ai[0] > 20 && !dead)) //If in first phase and projectile is not inactive
                    {
                        ...
                        if(((Projectile.ai[0] + Projectile.ai[1]) * 120) % Main.GameUpdateCount == 0)
                        {
                            ...
                            Projectile projectile2 = Projectile.NewProjectileDirect(new Terraria.DataStructures.ProjectileSource_TileInteraction(Main.LocalPlayer, i, j), Projectile.Center, Vector2.Normalize(target.Center - Projectile.Center) * 2, ModContent.ProjectileType<SpireLaserAlt>(), (int)(Main.npc[spire.whoAmI].damage / 5f), 0f, default, 0, 1);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (spire.active == true && Vector2.DistanceSquared(Main.LocalPlayer.Center, Projectile.Center) <= (192 * 16) * (192 * 16))
            {
                ...
                Texture2D tex = ModContent.GetInstance<EEMod>().Assets.Request<Texture2D>("Tiles/Foliage/Aquamarine/AquamarineLamp1Glow").Value;
