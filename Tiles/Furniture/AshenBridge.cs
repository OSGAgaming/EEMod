...
namespace EEMod.Tiles.Furniture
{
   ...
   float x0, float x1, float x2, float x3)
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (isSupport)
            {
                spriteBatch.Draw(ModContent.Request<Texture2D>("EEMod/Tiles/AshenBridgeSupport"), Projectile.Center - Main.screenPosition, new Rectangle(0, 0, 16, 16), lightColor, Projectile.rotation, new Vector2(16, 16) / 2, 1, SpriteEffects.None, 0);
                return false;
            }
            return true;
        }
