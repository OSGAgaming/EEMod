...
namespace EEMod.Seamap.SeamapContent
{
    class Island : SeamapObject
    {
        ...
        {
            if (name != null)
                if (!SeamapObjects.Islands.ContainsKey(name))
                    SeamapObjects.Islands.Add(name, this);
        }
        public Island(Vector2 position, int width, int height, Texture2D texture, string islandname, int framecount, int framespid, bool cancollide)
        {
