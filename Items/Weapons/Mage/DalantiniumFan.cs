...
namespace EEMod.Items.Weapons.Mage
{
    public class DalantiniumFan : EEProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            {
                Projectile.NewProjectile(new Terraria.DataStructures.ProjectileSource_Item(player, Item), player.Center + (direction2 * 5), new Vector2((float)Math.Sin(-radians - 1.57), (float)Math.Cos(-radians - 1.57)) * 10, ModContent.ProjectileType<DalantiniumFang>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
            }
